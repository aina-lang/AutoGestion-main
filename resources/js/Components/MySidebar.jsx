...
export default function Sidebar({ children, auth }) {
    ...
    return (
        <aside
            className={`h-screen  shadow-md bg-white transition-all ease-in dark:bg-gray-800 ${expanded ? 'w-60' : ''}`}
        >
            <nav className={`flex h-full flex-col bg-[${currentPalette[500]}]`}>
                ...
                <SidebarContext.Provider value={{ expanded }}>
                    <ul
                        className={`pt-5 h-20 overflow-hidden
                             flex-1 px-3 space-y-3 ${expanded ? 'overflow-y-auto' : ''}`}
                    >
                        {children}
                    </ul>
