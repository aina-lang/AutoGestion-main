    ...
    public function all(Request $request)
    {
        ...
        if ($searchTerm) {
            ...
            $filteredVehicles = $vehicles->filter(function ($vehicule) use ($searchTerm) {
                ...
                // Si le terme de recherche est court, utiliser la correspondance de préfixe
                if (strlen($searchTerm) <= 2) {
                    return str_starts_with(strtolower($vehicule->marque), strtolower($searchTerm)) ||
                           str_starts_with(strtolower($vehicule->modele), strtolower($searchTerm));
                }
            
                return $jaccardMarque >= 0.3 || $jaccardModele >= 0.3;
            });
